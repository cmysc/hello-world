...
print ('hw-start: min-min scheduling')
...
def min_min_schedual(data):
    ...
    tempRes = findMinInListOfDic(minTaskOnMachine, 'timeCost', 'task', 'machine', 'timeCost')
    finalMinTaskOverAllMachines = []
    for x in tempRes:
        finalMinTaskOverAllMachines.append(x)
    print 'Stage - the following MinTask is assigned:'
    print finalMinTaskOverAllMachines

    # -----------------  remove the min machine from data ---------------------
    dataRemoved = [d for d in data if d['task']not in [x['task'] for x in finalMinTaskOverAllMachines]]
    #print '-----------'
    ...
    # ----------------- update other task timeCost on the same min machine ---------------------
        ...
        for d in dataRemoved:
            ...
            if d['machine'] in [x['machine'] for x in finalMinTaskOverAllMachines]:
                for x in finalMinTaskOverAllMachines:
        ...
        result = []
        result.append(min_min_schedual(dataUpdated))
